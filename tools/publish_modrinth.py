...
means "loads on either side" and becomes `client_and_server`. MankeDoMath is a server mod that
...
PROJECT = os.environ.get("MODRINTH_ID", "Z8ToL0Ef")
...
def request(method, path, token, data=None, headers=None):
    ...
    req.add_header("User-Agent", "CodeW4VE/MankeDoMath (release script)")
...
def multipart(fields, files):
    ...
    boundary = "----mankedomath-release-boundary"
    body = b""
...
def main():
    ...
    version = mod_version()
    # Written out rather than linked to a GitHub tag: the tag is cut after the jars are known
    # to be good, so at upload time the link would be a 404.
    changelog = (
        "Builds for every Minecraft version from 1.21 to 26.2, from one source tree.\n\n"
        "The mod itself is unchanged. The whole port was two renames: the chat click and hover "
        "events became sealed interfaces in 1.21.5, and `GameProfile` became a record in 1.21.9. "
        "Everything else is the same code, because the calculator does not talk to Minecraft."
    )
    ...
    for minecraft in shipped:
        jar = WORK / f"mankedomath-{version}+{minecraft}.jar"
