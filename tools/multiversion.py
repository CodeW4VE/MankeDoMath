...
MankeDoMath is server side only, and nearly all of it is a parser that never mentions Minecraft
at all, so the churn that makes client mods expensive to port, the drawing layer being replaced
twice in a year, never touches it. What is left is renames in the three files that do talk to the
game, and a table of substitutions says them in one place better than the same file forked across
branches.
...
def jdk_for(version):
    ...
    candidates = [os.environ.get("MANKEDOMATH_JDK25"),
...
#
# The whole of math/ is missing from this table on purpose: it is a parser, a unit system and a
# formatter, and it does not import Minecraft. Only MankeDoMath, MathCommand and MathConfig talk
# to the game at all, so the table below is the entire cost of the port.

# 1.21.5 turned the chat click and hover events into sealed interfaces with a record per action.
# The action constant becomes the type, so the argument that named it goes away and the closing
# paren stays where it was, which is why these read as prefixes rather than whole expressions.
SINCE_1_21_5 = [
    ("new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, ", "new ClickEvent.SuggestCommand("),
    ("new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, ", "new ClickEvent.CopyToClipboard("),
    ("new HoverEvent(HoverEvent.Action.SHOW_TEXT,", "new HoverEvent.ShowText("),
]

# 1.21.9 is where GameProfile became a record, so its getters lost the get prefix. Only the name
# is read here, to say who shared an answer.
#
# Worth being exact about the version: our other mods have this rename filed under 1.21.11,
# because they skip 1.21.9 and 1.21.10 and 1.21.11 was simply the first build that saw it. Here
# those two are shipped, and they fail without this row.
SINCE_1_21_9 = SINCE_1_21_5 + [
    (".getGameProfile().getName()", ".getGameProfile().name()"),
]

# 1.21.11 made a command's permission a check object rather than a number. Level 2, what /math
# reload and the gated /math share ask for, is LEVEL_GAMEMASTERS.
#
# It comes in two shapes here and they need different answers. As the whole of a `requires` the
# check object goes in as it is. Inside a larger condition, where share is only gated when the
# config says so, there is no predicate slot to fill, so it has to be asked directly.
...
def build(version, api, errors_only=False):
    ...
    # No mixin config to bump to JAVA_25 here: the mod registers a command and reads a config
    # file, and neither needs to reach inside a vanilla class.
    if unobfuscated(version):
    ...
    out = WORK / f"mankedomath-{mod_version()}+{version}.jar"
